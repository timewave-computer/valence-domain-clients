...
async fn create_test_client() -> Result<GaiaClient, ClientError> {
    ...
    let chain_id =
        env::var("GAIA_CHAIN_ID").unwrap_or_else(|_| "cosmoshub-4".to_string());

    let mnemonic =
        env::var("GAIA_MNEMONIC").expect("GAIA_MNEMONIC must be set for tests");
...
async fn test_gaia_staking_queries() {
    ...
    assert!(
        distribution_account.is_ok(),
        "Failed to query distribution module account: {:?}",
        distribution_account.err()
    );

    println!(
        "Distribution module account: {:?}",
        distribution_account.unwrap()
    );
}
