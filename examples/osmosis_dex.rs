// Example demonstrating Osmosis DEX integration with valence-domain-clients
//-----------------------------------------------------------------------------
// Osmosis DEX Example
//-----------------------------------------------------------------------------
...
use valence_domain_clients::{
    ...
    cosmos::{chains::osmosis::OsmosisClient, types::CosmosCoin},
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if osmo_balance >= 1_000_000 {
        // 1 OSMO
        println!("Performing token swap...");

        // Execute a swap from OSMO to ATOM
        let swap_result =
            match swap_tokens(&osmosis, "uosmo", atom_denom, 1_000_000u128).await {
                Ok(tx) => {
