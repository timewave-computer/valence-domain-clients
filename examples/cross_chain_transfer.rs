...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let noble_mnemonic = "test test test test test test test test test test test junk";
    
    // Ethereum amount parameters - using new type constructor format
    let eth_amount = EvmU256([0, 0, 0, 10000000000000000]); // 0.01 ETH
    let eth_recipient = EvmAddress([0x74, 0x2d, 0x35, 0xCc, 0x66, 0x34, 0xC0, 0x53, 0x29, 0x25, 0xa3, 0xb8, 0x44, 0xBc, 0x45, 0x4e, 0x44, 0x38, 0xf4, 0x4e]);
