// Example of client for Base blockchain integration with valence-domain-clients
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Connected to Base network with chain ID: {}",
        client.network_chain_id()
    );
    ...
    if config_path.exists() {
        ...
        println!(
            "File-based client chain ID: {}",
            file_client.network_chain_id()
        );
    } else {
        println!(
        ...
        println!(
            "Run 'nix run .#fetch-protos -- --base' to generate the config file"
        );
    }
