...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mnemonic = env::var("NOBLE_MNEMONIC")
        .expect("NOBLE_MNEMONIC must be set");
    ...
    ).await.expect("Failed to create Noble client");
    
    //-----------------------------------------------------------------------------
    // Query account information
    //-----------------------------------------------------------------------------
    let signer = noble_client.get_signer_details().await?
        .address;
    println!("Connected to Noble as address: {}", signer);

    // Query USDC balance
    let balance = noble_client.query_balance(&signer.0, "uusdc").await?;
    println!("USDC balance: {} uusdc ({} USDC)", balance, balance as f64 / 1_000_000.0);
    ...
    let transfer_result = noble_client.transfer(
        to_address,
    ...
    let cctp_result = noble_client.cctp_deposit_for_burn(
        destination_domain,
        destination_address,
        transfer_amount,
        "uusdc",
    ...
    match cctp_result {
        Ok(result) => {
            ...
            println!("2. The message and attestation are submitted to the destination chain");
        },
    ...
    println!("Note: This example requires the message and attestation from a cross-chain transfer.");
    println!("      In a real application, you would obtain these from an attestation API or indexer.");
