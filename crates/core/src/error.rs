...
impl From<prost::EncodeError> for ClientError {
    fn from(err: prost::EncodeError) -> Self {
        ClientError::SerializationError(format!(
            "Protocol buffer encoding error: {err}"
        ))
    }
}
...
impl From<prost::DecodeError> for ClientError {
    fn from(err: prost::DecodeError) -> Self {
        ClientError::SerializationError(format!(
            "Protocol buffer decoding error: {err}"
        ))
    }
}
...
mod tests {
    ...
    fn test_client_error_creation() {
        ...
        let client_err =
            ClientError::ClientError("general client error".to_string());
        ...
        let not_impl_err =
            ClientError::NotImplemented("feature not available".to_string());
        let timeout_err = ClientError::TimeoutError("request timed out".to_string());
        let ser_err =
            ClientError::SerializationError("serialization failed".to_string());
        let config_err = ClientError::ConfigError("invalid config".to_string());
        let not_found_err =
            ClientError::NotFoundError("resource missing".to_string());
        let state_err = ClientError::StateMismatch("unexpected state".to_string());
        let action_err =
            ClientError::ActionFailed("action couldn't complete".to_string());
        let contract_err =
            ClientError::ContractError("contract execution failed".to_string());
        let abi_err = ClientError::AbiError("ABI encoding failed".to_string());
        let msg_type_err =
            ClientError::InvalidMsgType("wrong message type".to_string());
    ...
    fn test_client_error_messages() {
        ...
        let query_err = ClientError::QueryError("failed query".to_string());
        assert_eq!(query_err.to_string(), "query error: failed query");
        ...
        let timeout_err = ClientError::TimeoutError("request timed out".to_string());
        assert_eq!(timeout_err.to_string(), "timeout error: request timed out");
    }
