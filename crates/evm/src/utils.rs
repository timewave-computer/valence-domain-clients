...
// Use proper imports in the new workspace structure
use crate::crypto::keccak256;
use crate::types::{EvmAddress, EvmBytes, EvmHash, EvmTransactionReceipt, EvmU256};
use hex;
...
pub fn parse_hex_u256(hex_str: &str) -> Result<EvmU256, ClientError> {
    ...
    let bytes = hex::decode(clean_hex).map_err(|e| {
        ClientError::ParseError(format!("Failed to decode hex: {e}"))
    })?;
...
/// Returns the validated address if valid, or an error if invalid
pub fn validate_address(address: &str) -> Result<EvmAddress, ClientError> {
    // Remove 0x prefix if present
    let clean_address = if let Some(stripped) = address.strip_prefix("0x") {
        stripped
    } else {
        address
    };
    ...
    if clean_address.len() != 40 {
        return Err(ClientError::ParseError(format!(
            "Invalid Ethereum address length: {} characters (expected 40)",
            clean_address.len()
        )));
    }

    // Validate hex content
    let bytes = hex::decode(clean_address).map_err(|e| {
        ClientError::ParseError(format!("Invalid Ethereum address format: {e}"))
    })?;

    if bytes.len() != 20 {
        return Err(ClientError::ParseError(
            "Decoded address length is not 20 bytes".to_string(),
        ));
    }
...
pub fn parse_chain_id(chain_id: &str) -> Result<u64, ClientError> {
    match chain_id {
        ...
        _ => {
            ...
            chain_id.parse::<u64>().map_err(|e| {
                ClientError::ParseError(format!("Invalid chain ID: {e}"))
            })
        }
    }
}
...
/// Formats an address as a checksum address according to EIP-55
pub fn to_checksum_address(address: &EvmAddress) -> String {
    // Convert to lowercase hex without 0x prefix
    let address_hex = hex::encode(address.as_bytes());
    let address_lowercased = address_hex.to_lowercase();

    // Hash the lowercase address
    let hash = keccak256(address_lowercased.as_bytes());
    let hash_hex = hex::encode(hash);

    // Iterate through each character
    let mut checksum_address = String::with_capacity(42); // 0x + 40 hex chars
    checksum_address.push_str("0x");

    for (i, ch) in address_lowercased.chars().enumerate() {
        if ('a'..='f').contains(&ch) {
            // Get the corresponding hash nibble (4 bits)
            let hash_char = hash_hex.chars().nth(i).unwrap_or('0');

            // If the hash character value >= 8, uppercase the address character
            if hash_char >= '8' {
...
pub fn is_valid_address(address: &str) -> bool {
    ...
    // Address should be 42 characters (0x + 40 hex characters)
    if address.len() != 42 {
        return false;
    }
...
/// Validates an Ethereum transaction hash format
...
/// Gets transaction status from a receipt
pub fn get_transaction_status(
    receipt: &EvmTransactionReceipt,
) -> Result<bool, ClientError> {
    // Status is a u8 where 1 means success and 0 means failure
    Ok(receipt.status == 1)
}

/// Safely converts a hex string to a U256
pub fn safe_hex_to_u256(hex: &str) -> Result<EvmU256, ClientError> {
    let clean_hex = hex.strip_prefix("0x").unwrap_or(hex);

    if clean_hex.is_empty() {
        return Ok(EvmU256::zero());
    }
    ...
    bytes_arr[start_idx..].copy_from_slice(&bytes);

    EvmU256::from_big_endian(&bytes_arr).map_err(|e| {
        ClientError::ClientError(format!("Failed to convert bytes to U256: {e}"))
    })
}

/// Converts an address to bytes
pub fn address_to_bytes(address: &EvmAddress) -> &[u8; 20] {
    &address.0
}

/// Get a transaction receipt URL for a specific chain explorer
pub fn get_transaction_receipt_url(chain_id: u64, tx_hash: &EvmHash) -> String {
    let hash_hex = format!("0x{}", hex::encode(tx_hash.0));

    match chain_id {
        1 => format!("https://etherscan.io/tx/{hash_hex}"),
        5 => format!("https://goerli.etherscan.io/tx/{hash_hex}"),
        11155111 => format!("https://sepolia.etherscan.io/tx/{hash_hex}"),
        8453 => format!("https://basescan.org/tx/{hash_hex}"),
        84531 => format!("https://sepolia.basescan.org/tx/{hash_hex}"),
        84532 => format!("https://goerli.basescan.org/tx/{hash_hex}"),
        _ => format!("https://blockscan.com/tx/{hash_hex}"),
    }
}

/// Parse number from an Ethereum quantity string (e.g. "0x1a" -> 26)
pub fn parse_eth_quantity(quantity: &str) -> Result<u64, ClientError> {
    // Handle "0x" prefix
    let clean_quantity = quantity.strip_prefix("0x").unwrap_or(quantity);

    // Handle empty string or "0x"
    if clean_quantity.is_empty() {
        return Ok(0);
    }

    // Parse as hex
    let value = u64::from_str_radix(clean_quantity, 16).map_err(|e| {
        ClientError::ParseError(format!("Failed to parse Ethereum quantity: {e}"))
    })?;

    Ok(value)
}

/// Format number as an Ethereum quantity string (e.g. 26 -> "0x1a")
pub fn format_eth_quantity(value: u64) -> String {
    if value == 0 {
...
/// Validate a new transaction receipt
pub fn validate_transaction_receipt(
    receipt: &EvmTransactionReceipt,
) -> Result<(), ClientError> {
    // Check transaction hash
    if receipt.transaction_hash.0.is_empty() {
        return Err(ClientError::ClientError(
            "Transaction receipt has empty transaction hash".to_string(),
        ));
    }
