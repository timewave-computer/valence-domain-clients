...
//! Flashbots bundle support for EVM clients.
//!
//! This module provides functionality for creating and sending Flashbots bundles
//! which allow for gas price auction bypass and private transaction ordering.
...
/// Representation of a Flashbots bundle
#[derive(Clone, Debug)]
pub struct FlashbotsBundle {
    /// The transactions that make up this bundle
    pub transactions: Vec<EvmBytes>,
    /// The block number this bundle targets
    pub target_block_number: u64,
    /// The minimum timestamp for this bundle
    pub min_timestamp: Option<u64>,
    /// The maximum timestamp for this bundle
    pub max_timestamp: Option<u64>,
    /// URL of the relay to use
    pub relay_url: String,
}

/// Interface for sending Flashbots bundles
#[async_trait]
pub trait FlashbotsBundleSender: Send + Sync {
    /// Sign and send a bundle to the Flashbots relay
    async fn send_bundle(
        &self,
        bundle: &FlashbotsBundle,
    ) -> Result<FlashbotsResponse, ClientError>;

    /// Simulate a bundle through the Flashbots relay
    async fn simulate_bundle(
        &self,
        bundle: &FlashbotsBundle,
    ) -> Result<FlashbotsSimulationResponse, ClientError>;
}

/// The response from sending a bundle to the Flashbots relay
...
/// The response from simulating a bundle through the Flashbots relay
#[derive(Debug, Clone, Deserialize)]
pub struct FlashbotsSimulationResponse {
    /// Flag indicating if the simulation succeeded
    #[serde(rename = "simulationSucceeded")]
    pub simulation_succeeded: bool,
    /// The transaction hashes that were successfully simulated
    #[serde(rename = "hashes")]
    ...
    #[serde(rename = "simulationErrors")]
    pub simulation_errors: Option<SimulationErrors>,
}

/// Errors that can occur during Flashbots bundle simulation
#[derive(Debug, Clone, Deserialize)]
...
/// Bundle request payload for sending to the Flashbots relay
#[derive(Debug, Serialize)]
pub struct FlashbotsBundleRequest {
    /// Transaction bytes in hex format
    #[serde(rename = "txs")]
    pub transactions: Vec<String>,
    /// Target block number
    #[serde(rename = "blockNumber")]
    pub block_number: String,
    /// Bundle minimum timestamp
    #[serde(rename = "minTimestamp", skip_serializing_if = "Option::is_none")]
    pub min_timestamp: Option<String>,
    /// Bundle maximum timestamp
...
/// Implementation of FlashbotsBundle
impl FlashbotsBundle {
    /// Create a new FlashbotsBundle
    pub fn new(transactions: Vec<EvmBytes>, target_block_number: u64) -> Self {
        Self {
            transactions,
            target_block_number,
            min_timestamp: None,
    ...
    /// Set the bundle's target block number
    pub fn set_target_block(&mut self, block_number: u64) -> &mut Self {
        self.target_block_number = block_number;
        self
    }
}
...
#[async_trait]
pub trait FlashbotsBundleOperations {
...
pub fn create_mev_bundle(
    ...
    MevSendBundleParams {
        ...
        privacy: if hints.is_some() || builders.is_some() {
            Some(PrivacyConfig { hints, builders })
        } else {
            None
        },
        metadata: None,
...
pub fn create_mev_share_bundle(
    ...
    let mut bundle =
        create_mev_bundle(transactions, target_block, max_block, hints, builders);
...
mod tests {
    ...
    fn test_create_mev_bundle() {
        ...
        let builders =
            Some(vec!["flashbots".to_string(), "builder0x69".to_string()]);
    ...
    fn test_create_mev_share_bundle() {
        ...
        let refund_addresses =
            vec![("0x1234567890123456789012345678901234567890".to_string(), 90)];
