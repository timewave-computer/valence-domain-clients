...
use crate::core::transaction::{TransactionResponse, Event};
...
impl GenericEvmClient {
    ...
    pub async fn sign_transaction(&self, _tx: &EvmTransactionRequest) -> Result<EvmBytes, ClientError> {
        Err(ClientError::NotImplemented("Signing not implemented in generic client".to_string()))
    }
    ...
    fn receipt_to_response(&self, receipt: Value) -> Result<TransactionResponse, ClientError> {
        let tx_hash = receipt["transactionHash"].as_str()
            .ok_or_else(|| ClientError::ParseError("Missing transaction hash".to_string()))?
            .to_string();
...
impl EvmBaseClient for GenericEvmClient {
    ...
    async fn wait_for_transaction_receipt(&self, tx_hash: &EvmHash) -> Result<TransactionResponse, ClientError> {
        ...
        Err(ClientError::TimeoutError("Transaction not mined within timeout".to_string()))
    }
...
impl Provider {
    ...
    async fn call<T: serde::de::DeserializeOwned>(&self, method: &str, params: Value) -> Result<T, String> {
        let request = json!({
            "jsonrpc": "2.0",
            "id": 1,
            "method": method,
