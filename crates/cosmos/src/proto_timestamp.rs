...
#[allow(dead_code)]
impl ProtoTimestamp {
    pub(crate) fn extend_by_seconds(&self, seconds: i64) -> Self {
    ...
    pub(crate) fn now() -> Self {
        ...
        let nanos = now.timestamp_nanos_opt().unwrap_or(0) % NANOS_IN_SECOND as i64;
    ...
    pub(crate) fn to_nanos(&self) -> Result<u64, ClientError> {
        ...
        if seconds > u64::MAX / NANOS_IN_SECOND {
            return Err(ClientError::ParseError(
                "Timestamp seconds overflow".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_proto_timestamp_now() {
        ...
        assert!(
            diff <= 1,
            "Timestamps should be within 1 second, but differ by {} seconds",
            diff
        );
    }
