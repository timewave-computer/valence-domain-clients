...
use cosmos_sdk_proto::cosmos::tx::v1beta1::{
    AuthInfo as ProtoAuthInfo, SignerInfo as ProtoSignerInfo, Tx as ProtoTx,
    TxBody as ProtoTxBody,
};
use cosmos_sdk_proto::cosmos::tx::v1beta1::{
...
use crate::{
    ...
    types::{
        CosmosAccount, CosmosAccountPubKey, CosmosBaseAccount, CosmosBlockResults,
        CosmosCoin as CosmosTypesCoin, CosmosConsensusVersion, CosmosFee,
        CosmosGasInfo, CosmosHeader, CosmosModuleAccount, CosmosSimulateResponse,
...
// Add the utils module to imports
use crate::utils::{bytes_to_string, convert_from_proto_tx_response};
...
impl GrpcSigningClient for GenericCosmosClient {
        ...
        Ok(CosmosSimulateResponse {
            simulate_result: sim_res_inner
                ...
                .map(|res| CosmosSimulateResult {
                    ...
                    log: res.log,
                    events: {
                        // Convert v0_34 StringEvents to our event type directly
                        res.events
                            .into_iter()
                            .map(|evt| transaction::Event {
                                event_type: evt.r#type,
                                attributes: evt
                                    .attributes
                                    .into_iter()
                                    .map(|attr| {
                                        // Convert Bytes to String using helper function
                                        (
                                            bytes_to_string(&attr.key),
    ...
    async fn sign_and_broadcast_tx(
        ...
        fee: CosmosFee,
        memo: Option<&str>,
    ) -> Result<valence_core::transaction::TransactionResponse, ClientError> {
        ...
        let our_tx_bytes = sc
            .sign_tx(msg, fee.clone(), memo, &chain_default_denom)
            .await?;
        ...
        let broadcast_res = tx_service_client
            ...
            .await
            .map_err(|e| {
                ClientError::ClientError(format!("Failed to broadcast tx: {e}"))
            })?
            .into_inner();
...
#[async_trait::async_trait]
impl crate::base_client::CosmosBaseClient for GenericCosmosClient {
    ...
    async fn query_module_account(
        ...
        Err(ClientError::NotImplemented(
            "query_module_account".to_string(),
        ))
    }
    ...
    async fn poll_until_expected_balance(
        ...
        Err(ClientError::NotImplemented(
            "poll_until_expected_balance".to_string(),
        ))
    }
    ...
    async fn ibc_transfer(
        ...
        let amount_u128 = amount.parse::<u128>().map_err(|e| {
            ClientError::ParseError(format!(
                "Failed to parse amount '{amount}': {e}"
            ))
        })?;
        ...
        let timeout_timestamp = chrono::Utc::now()
            ...
            .ok_or_else(|| {
                ClientError::ParseError(
                    "Failed to calculate timeout timestamp".to_string(),
                )
            })?
            .timestamp_nanos_opt()
            .unwrap_or(0);
        ...
        // Sign and broadcast
        self.sign_and_broadcast_tx(any_msg, fee, memo.as_deref())
            .await
    }
}
...
impl GenericCosmosClient {
    ...
    pub async fn get_signer_details(&self) -> Result<CosmosAccount, ClientError> {
        let sc = self.signing_client.lock().await;
    ...
    pub async fn transfer(
        ...
        let denom_parsed: CosmrsDenom = denom_str.parse().map_err(|e| {
            ClientError::ParseError(format!("Invalid denom '{denom_str}': {e}"))
        })?;
    ...
    pub async fn latest_block_header(&self) -> Result<CosmosHeader, ClientError> {
            ...
            match client.get_tx(request).await {
                Ok(response) => {
                    ...
                    if let Some(tx_response_proto) = get_tx_response.tx_response {
                        // Use our utility function to convert the proto response
                        return convert_from_proto_tx_response(tx_response_proto);
                    }
