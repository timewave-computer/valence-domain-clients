...
impl NeutronClient {
    ...
    pub async fn execute_contract<T: Serialize + Send>(
        ...
        let msg_bytes = serde_json::to_vec(msg).map_err(|e| {
            ClientError::ParseError(format!("Failed to serialize message: {e}"))
        })?;

        // Get the signer details
        let account = self.get_signer_details().await?;

        // Parse sender address and contract address
        let sender = match account.address.0.parse::<cosmrs::AccountId>() {
            Ok(id) => id,
            Err(e) => {
        ...
        let contract = match contract_address.parse() {
            Ok(addr) => addr,
            Err(e) => {
                return Err(ClientError::ParseError(format!(
        ...
        // Convert to Any
        let any_msg = match execute_msg.to_any() {
            Ok(msg) => msg,
            Err(e) => {
                return Err(ClientError::ClientError(format!(
                    "Failed to convert message to Any: {e}"
                )))
            }
        };
    ...
    >(
        ...
        let query_msg_string = serde_json::to_string(query_msg).map_err(|e| {
            ClientError::ParseError(format!(
                "Failed to serialize query message: {e}"
            ))
        })?;
        ...
        let request =
            cosmos_sdk_proto::cosmwasm::wasm::v1::QuerySmartContractStateRequest {
                address: contract_address.to_string(),
                query_data: query_msg_string.into_bytes(),
            };

        // Execute the query
        let response = wasm_query_client
            .smart_contract_state(request)
            .await
            .map_err(|e| {
                ClientError::ClientError(format!(
                    "Failed to query contract state: {e}"
                ))
            })?;

        // Deserialize the response
        serde_json::from_slice(&response.get_ref().data).map_err(|e| {
            ClientError::ParseError(format!(
                "Failed to deserialize contract response: {e}"
            ))
        })
    }
}
