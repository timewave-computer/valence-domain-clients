...
impl GaiaClient {
    ...
    pub async fn delegate(
        ...
        let delegator: cosmrs::AccountId = self
            .get_signer_details()
            .await?
            .address
            .0
            ...
            .map_err(|e| {
                ClientError::ParseError(format!(
                    "Failed to parse delegator address: {e}"
                ))
            })?;

        let _validator = validator_address.parse::<String>().map_err(|e| {
            ClientError::ParseError(format!(
                "Failed to parse validator address: {e}"
            ))
        })?;
