...
use crate::core::types::GenericAddress;
...
pub struct CosmosSimulateResult {
    ...
    pub log: String,
    pub events: Vec<crate::core::transaction::Event>,
}
...
pub struct CosmosBlockResults {
    pub height: u64,
    pub txs_results: Option<Vec<crate::core::transaction::TransactionResponse>>,
    pub begin_block_events: Option<Vec<crate::core::transaction::Event>>,
    pub end_block_events: Option<Vec<crate::core::transaction::Event>>,
...
impl Default for FeeSetting {
    fn default() -> Self {
        FeeSetting::Auto { gas_adjustment: 1.2 }
    }
}
...
mod tests {
    ...
    fn test_cosmos_account_pubkey_variants() {
        ...
        match deserialized {
            CosmosAccountPubKey::Secp256k1(bytes) => assert_eq!(bytes, vec![1, 2, 3, 4]),
        ...
        let deserialized: CosmosAccountPubKey = serde_json::from_str(&json).unwrap();
        match deserialized {
            CosmosAccountPubKey::Ed25519(bytes) => assert_eq!(bytes, vec![5, 6, 7, 8]),
    ...
    fn test_cosmos_broadcast_tx_request() {
        let request = CosmosBroadcastTxRequest {
            ...
            fee_setting: FeeSetting::Fixed { 
                fee_amount: vec![CosmosCoin { denom: "uatom".to_string(), amount: 5000 }], 
                gas_limit: 300000 
            },
        ...
        let deserialized: CosmosBroadcastTxRequest = serde_json::from_str(&json).unwrap();
        ...
        match request.fee_setting {
            FeeSetting::Fixed { fee_amount, gas_limit } => {
