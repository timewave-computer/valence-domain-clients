...
use crate::cosmos::types::{
    CosmosAccount, CosmosBlockResults, CosmosFee,
    CosmosHeader, CosmosModuleAccount, CosmosSimulateResponse,
};
...
impl GrpcSigningClient for NobleClient {
    ...
    async fn query_chain_gas_config(&self, _chain_name: &str, _chain_denom: &str) -> Result<f64, ClientError> {
...
impl CosmosBaseClient for NobleClient {
    async fn transfer(
        ...
        self.inner.transfer(to_address_str, amount, denom, memo).await
    }
    ...
    async fn poll_until_expected_balance(
        ...
        self.inner.poll_until_expected_balance(
            address, denom, min_amount, interval_sec, max_attempts
        ).await
    }
