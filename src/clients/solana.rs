...
use crate::solana::{
    ...
    signing_client::{SolanaClient, SolanaSigningClient},
};
...
mod tests {
    ...
    async fn test_localnet_transfer() {
        ...
        let transfer_result = client1
            .transfer_sol_amount(&client2.get_pubkey_string(), 0.5)
            .await
            .unwrap();
    ...
    async fn test_localnet_account_operations() {
        ...
        let exists = client
            .account_exists(&client.get_pubkey_string())
            .await
            .unwrap();
        assert!(exists);
    ...
    async fn test_localnet_poll_balance() {
        ...
        let client1_clone =
            SolanaTestClient::from_bytes(&client1.get_keypair().to_bytes()).unwrap();

        tokio::spawn(async move {
            tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
            let _ = client1_clone
                .transfer_sol_amount(&client2_pubkey, 0.5)
                .await;
        });

        // Poll for balance
        let balance = client2
            .poll_until_expected_sol_balance(&client2.get_pubkey_string(), 0.1, 1, 10)
            .await
            .unwrap();
        assert!(balance >= 0.1);
    }
