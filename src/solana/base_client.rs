...
pub trait SolanaBaseClient: SolanaSigningClient {
    ...
    async fn poll_for_tx_confirmation(
        ...
        let confirmed = self
            .poll_for_signature_confirmation(signature, timeout_seconds)
            .await?;
    ...
    async fn poll_until_expected_sol_balance(
        ...
        info!(
            "Polling {} SOL balance to exceed {}",
            address, min_amount_sol
        );
    ...
    async fn get_recent_performance_samples(
        &self,
    ) -> anyhow::Result<Vec<solana_rpc_client_api::response::RpcPerfSample>> {
        let rpc_client = self.get_rpc_client();
    ...
    async fn get_cluster_nodes(
        &self,
    ) -> anyhow::Result<Vec<solana_rpc_client_api::response::RpcContactInfo>> {
        let rpc_client = self.get_rpc_client();
