...
pub trait SolanaRpcClient {
        ...
        match rpc_client.get_account(pubkey).await {
            ...
            Err(e) => {
                ...
                if error_str.contains("AccountNotFound")
                    || error_str.contains("Invalid param: could not find account")
                {
                    Ok(None)
                } else {
                    Err(e.into())
    ...
    async fn confirm_transaction(&self, signature: &Signature) -> anyhow::Result<bool> {
        ...
        let result = rpc_client
            .confirm_transaction_with_spinner(signature, &self.commitment())
            .await?;
        Ok(result)
    }
    ...
    ) -> anyhow::Result<Option<solana_transaction_status::EncodedConfirmedTransactionWithStatusMeta>>
    {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                let error_str = e.to_string();
                if error_str.contains("Signature not found")
